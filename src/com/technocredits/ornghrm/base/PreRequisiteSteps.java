...
public class PreRequisiteSteps {
	...
	protected WebElement getElementInsideElement(String locatorType, String locator, boolean isWaitRequired, WebElement parentElement, String parentLocator) {
		WebElement element;

		switch (locatorType.toUpperCase()) {
		case "XPATH":
			if (isWaitRequired) {
				element = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(parentLocator+"//"+locator)));
				wait.until(ExpectedConditions.textMatches(By.xpath(parentLocator+"//"+locator), Pattern.compile("^[^0]")));
			}else
	...
	protected void entertext(String locatorType, String locator, boolean isWaitRequired, String text)
		...
		if (element.isEnabled())
			...
			throw new ElementNotEnabledException(
					"Element not enabled for Locator- " + locator + " and locator Type- " + locatorType);
	}
